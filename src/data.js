export const categories = [
  {
    id: 'academics',
    title: 'Academics',
    committees: [
      { id: ' Academic Planning ', name: 'Academic Planning ' },
      { id: 'Library Advisory', name: 'Library Advisory' },
      { id: 'Industry Institute Interaction', name: 'Industry Institute InteractiOn' },
  },
  {
    id: 'admisions',
    title: 'Admissions',
    committees: [
      { id: 'Scholarship', name: 'Scholarship' },
      { id: 'Student Welfare', name: 'Student Welfare' },
      { id: 'SC/ST', name: 'SC/ST' },
      { id: 'Anti-Ragging', name: 'Anti-Ragging' },
    ]
  },
  {
    id: 'administration',
    title: 'Administration',
    committees: [
      { id: 'Hostel Welfare', name: 'Hostel Welfare' },
      { id: 'Discipline', name: 'Discipline' },
      { id: 'Staff Selection', name: 'Staff Selection' },
      { id: 'Governing Body', name: 'Governing Body' },
      { id: 'Transportation', name: 'Transportation' }
  },
  {
    id: 'reasearch & development',
    title: 'Dean Reaserch & Development',
    committees: [
      { id: 'Skill Development', name: 'Skill Development' },
      { id: 'Research', name: 'Research' },
      { id: 'Enterpreneurship Development', name: 'Enterpreneurship Development' },
      { id: 'Patents/Startups', name: 'Patents/Startups' },
      { id: 'Women Empowerment', name: 'Women Empowerment' }
  },
  {
    id: 'internal quality assurance cell',
    title: ' Internal Quality Assurance Cell',
    committees: [
      { id: 'AICTE,NIRF,FFC,NBA,NAAC', name: 'AICTE,NIRF,FFC,NBA,NAAC' },
      { id: 'Academic Audit', name: 'Academic Audit' },
      { id: 'ISO', name: 'ISO' },
      { id: 'Grievance Redressal', name: 'Grievance Redressal' },
      { id: 'Internal Complaints', name: 'Internal Complaints' }
  },
   {
    id: ' Training & Placement',
    title: ' Training & Placement',
    committees: [
      { id: 'Training', name: 'Training' },
      { id: 'Placement', name: 'Placement' },
      { id: 'Alumni', name: 'Alumni' },
  },
   {
    id: ' planning & infrastructure',
    title: ' Planning & Infrastructure',
    committees: [
      { id: 'Infrastructure Development', name: 'Infrastructure Development' },
      { id: 'Purchase', name: 'Purchase' },
      { id: 'Finance', name: 'Finance' },
      { id: 'Electrical Maintenance', name: 'Electrical Maintenance' }
  },
  {
    id: ' student affairs',
    title: ' Student affairs',
    committees: [
      { id: 'NSS/NCC', name: 'NSS/NCC' },
      { id: 'Games/Sports', name: 'Games/Sports' },
      { id: 'SAC', name: 'SAC' }
  },
  {
    id: ' it services',
    title: ' IT Services',
    committees: [
      { id: 'Institute Magazine', name: 'Institute Magazine' },
      { id: 'Website Maintenance', name: 'Website Maintenance' },
      { id: 'System Administrator', name: 'System Administrator' }
  }  
];
export const committeeDetails = {
  'Industry  Institute Interaction Committe': {
    name: 'Industry Institute Interaction Committe',
    composition: [
      {
        role: 'Chairman',
        name: 'Dr.Radhika Paturi ',
        designation: 'Principal',
        phone: '+91-9866342224',
        email: 'vnits.j@gmail.com'
      },
      {
        role: 'Dean',
        name: 'Dr. G. sandhya',
        designation: 'Professor&HOD-ECE',
        phone: '+91-6300956881',
        email: 'sandhyag406@gmail.com'
      }
      {
        role: 'Main Coordinator',
        name: 'Dr.S.Yallamandaiah',
        designation: 'Associate Professor - ECE',
        phone: '+91-9959587120',
        email: 'yallamandaiah@gmail.com'
      }
    ],
    facultyMembers: [
      {
        name: 'A.Yugandhar Reddy ',
        designation: 'Associate Professor-CSE',
        phone: '+91-9392517845',
        email: 'yugandhar.akkala@gmail.com'
      },
      {
        name: 'M.Anusha',
        designation: 'Assistant Professor - CSM',
        phone: '+91-8374863576',
        email: 'anusharanimedarametla@gmail.com'
      },
      {
        name: 'M.Anusha ',
        designation: 'Assistant Professor - CSD',
        phone: '+91-8374863576',
        email: 'anusharanimedarametla@gmail.com'
      },
      {
        name: 'K.Srikanth',
        designation: 'Assistant Professor - IT',
        phone: '+91-9347489785',
        email: 'sreekilaru@gmail.com'
      },
      {
        name: 'N.Laxmi Narayana ',
        designation: 'ASST PROF- EEE',
        phone: '91-9553959485',
        email: 'nelluri.22@gmail.com'
      },
      {
        name: 'Dr.L.Vijaya ',
        designation: 'Assistant Professorn - MBA',
        phone: '+91-9985744456',
        email: 'vijaya.sivakiran@gmail.com'
      }
    ],
  }
};
export const committeeDetails = {
  'Institute Magazine Committe': {
    name: 'Institute Magazine Committe',
    composition: [
      {
        role: 'Chairman',
        name: 'Dr.Radhika Paturi ',
        designation: 'Principal',
        phone: '+91-9866342224',
        email: 'vnits.j@gmail.com'
      },
      {
        role: 'Dean',
        name: 'Dr.KVS Rama Krishna',
        designation: 'Assistant Professor & HOD - BS&H',
        phone: '+91-9963903911',
        email: 'ksai.mb@gmail.com'
      }
      {
        role: 'Main Coordinator',
        name: 'B.Rajashekar',
        designation: '-BS&H',
        phone: '+91-9951055081',
        email: 'prajasekar99@gmail.com'
      }
    ],
    facultyMembers: [
      {
        name: 'Dr. A.Naresh',
        designation: 'Professor- CSE',
        phone: '+91-9966135989',
        email: 'alapatinaresh13@gmail.com'
      },
      {
        name: 'G.Manidheer Babu',
        designation: 'Assistant Professor - CSM',
        phone: '+91-8985865601',
        email: 'manidheer.gorikapudi@gmail.com'
      },
      {
        name: 'G.Manidheer Babu ',
        designation: 'Assistant Professor - CSD',
        phone: '+91-8985865601',
        email: 'manidheer.gorikapudi@gmail.com'
      },
      {
  
        name: 'K.Murali Krishna ',
        designation: 'Assistant Professor - ECE',
        phone: '+91-8074113694',
        email: 'murali.kondaveeti435@gmail.com'
      },

      {
        name: 'Sk.Sharmila ',
        designation: 'Assistant Professor - IT',
        phone: '+91-8309419583',
        email: 'mail2shaiksharmila@gmail.com'
      }
      {
        name: 'Dr.R.Srikanth ',
        designation: 'ASSOC PROF- EEE',
        phone: '91-9700445084',
        email: 'ravipatister@gmail.com'
      }
      {
        name: 'Dr.M.Naga Lakshmi ',
        designation: 'Assistant Professor - MBA',
        phone: '+91-9700229394',
        email: 'lakshmirafi1997@gmail.com'
      }
    ],
  }
};
export const committeeDetails = {
  'Website Maintanance Committe': {
    name: 'Website Maintanance Committe',
    composition: [
      {
        role: 'Chairman',
        name: 'Dr.Radhika Paturi ',
        designation: 'Principal',
        phone: '+91-9866342224',
        email: 'vnits.j@gmail.com'
      },
      {
        role: 'Dean',
        name: 'Dr.KVS Rama Krishna',
        designation: 'Assistant Professor & HOD - H&S',
        phone: '+91-9963903911',
        email: 'ksai.mb@gmail.com'
      }
      {
        role: 'Main Coordinator',
        name: 'K.Chaitanya',
        designation: 'Associate Professor-CSE',
        phone: '+91-9603956946',
        email: 'chaitanyacns3@gmail.com'
      }
    ],
    facultyMembers: [
      {
        name: 'Syed Firdous Fathima',
        designation: 'Assistant Professor - CSM',
        phone: '+91-9381448189',
        email: 'skff810@gmail.com'
      },
      {
        name: 'Syed Firdous Fathima',
        designation: 'Assistant Professor - CSD',
        phone: '+91-9381448189',
        email: 'skff810@gmail.com'
      },
      {
        name: 'M.Kranthi Kumar',
        designation: 'Assistant Professor - ECE',
        phone: '+91-9493241272',
        email: 'kranthi.3019@gmail.com'
      },
      {
        name: 'Sk.Sharmila ',
        designation: 'Assistant Professor - IT',
        phone: '+91-8309419583',
        email: 'mail2shaiksharmila@gmail.com'
      },

      {
        name: 'Dr.R.Srikanth ',
        designation: 'Assistant Professor - EEE',
        phone: '+91-9700445084',
        email: 'ravipatister@gmail.com'
      }
      {
        name: 'B.Kiran Kumar ',
        designation: 'Assistant Professor - BS&H',
        phone: '+91-8096565120',
        email: 'bandikirankumar1807@gmail.com'
      }
      {
        name: 'Dr.M.Naga Lakshmi ',
        designation: 'Assistant Professor - MBA',
        phone: '+91-9700229394',
        email: 'lakshmirafi1997@gmail.com'
      }
    ],
  }
};
export const committeeDetails = {
  'System Administrator Committe': {
    name: 'System Administrator Committe',
    composition: [
      {
        role: 'Chairman',
        name: 'Dr.Radhika Paturi ',
        designation: 'Principal',
        phone: '+91-9866342224',
        email: 'vnits.j@gmail.com'
      },
      {
        role: 'Dean',
        name: 'Dr.KVS Rama Krishna',
        designation: 'Assistant Professor & HOD - H&S',
        phone: '+91-9963903911',
        email: 'ksai.mb@gmail.com'
      }
      {
        role: 'Main Coordinator',
        name: 'Yugandhar Reddy',
        designation: 'Associate Professor-CSE',
        phone: '+91-9392517845',
        email: 'yugandhar.akkala@gmail.com'
      }
    ],
    facultyMembers: [
      {
        name: 'G.Manidheer Babu',
        designation: 'Assistant Professor - CSM',
        phone: '+91-8985865601',
        email: 'manidheer.gorikapudi@gmail.com'
      },
      {
        name: 'G.Manidheer Babu',
        designation: 'Assistant Professor - CSD',
        phone: '+91-8985865601',
        email: 'manidheer.gorikapudi@gmail.com'
      },
      
      {
        name: 'M.Kranthi Kumar',
        designation: 'Assistant Professor - ECE',
        phone: '+91-9493241272',
        email: 'kranti3019@gmail.com'
      },
      {
        name: 'G.Kusuma Harinath ',
        designation: 'Assistant Professor - IT',
        phone: '+91-9052526465',
        email: 'nirulagkhari@gmail.com'
      },
      {
        name: 'T.Rajesh ',
        designation: 'ASST PROF- EEE',
        phone: '+91-7207398722',
        email: 'thangellaraja232@gmail.com'
      }
      {
        name: 'V.China Annanda Rao ',
        designation: 'Assistant Professor - BS&H',
        phone: '+91-9705828682',
        email: 'anandphysics1984@gmail.com'
      }
       {
        name: 'M.Sarawati ',
        designation: 'Assistant Professor - MBA',
        phone: '+91-9398901719',
        email: 'saraswatinagaraju2011@gmail.com'
      }
    ],
  }
};
export const committeeDetails = {
  'Library Advisory Committee': {
    name: ' Library Advisory Committee',
    composition: [
      {
        role: 'Chairman',
        name: 'Dr.Radhika Paturi ',
        designation: 'Principal',
        phone: '+91-9866342224',
        email: 'vnits.j@gmail.com'
      },
      {
        role: 'Dean',
        name: 'Dr.G.Sandhya',
        designation: 'Professor&HOD-ECE',
        phone: '+91-6300956881',
        email: 'sandhyag406@gmail.com'
      }
      {
        role: 'Main Coordinator',
        name: 'Dr.Joseph Sanam',
        designation: 'Assistant Professor - EEE',
        phone: '+91-',
        email: 'joseph.nitr@gmail.com'
      }
    ],
    facultyMembers: [
      {
        name: 'M.Satya Vijaya ',
        designation: 'Associate Professor- CSE',
        phone: '+91-9490283200',
        email: 'satyavijaya.m@gmail.com'
      },
      {
        name: 'V.Manogna',
        designation: 'Assistant Professor - CSM',
        phone: '+91-7671825635',
        email: 'manusri1485@gmail.com'
      },
      {
        name: 'V.Manogna ',
        designation: 'Assistant Professor - CSD',
        phone: '+91-7671825635',
        email: 'manusri1485@gmail.com'
      },
      {
        name: 'Y.Vamsi Krishna Teja',
        designation: 'Assistant Professor - IT',
        phone: '+91-8309705324',
        email: 'vamsiyakkala@gmail.com'
      },
      {
        name: 'R.Srikanth',
        designation: 'Associate professor- EEE',
        phone: '+91-9700445084',
        email: 'ravipatister@gmail.com'
      },
     {
        name: 'A.Srinivasa Rao',
        designation: 'Assistant Professor - BS&H',
        phone: '+91-9441026791',
        email: 'aresrinivasrao90@gmail.com'
      },
      {
        name: 'P.Rupavathi',
        designation: 'Assistant Professor - MBA',
        phone: '+91-8555956850',
        email: 'rupa.puttala@gmail.com'
      }
    ],
  }
};
export const committeeDetails = {
  'Skill Development ': {
    name: ' Skill Development Committee',
    composition: [
      {
        role: 'Chairman',
        name: 'Dr.Radhika Paturi ',
        designation: 'Principal',
        phone: '+91-9866342224',
        email: 'vnits.j@gmail.com'
      },
      {
        role: 'Dean',
        name: 'Dr.V.Lakshman Narayana',
        designation: 'Professor&HOD- CSE',
        phone: '+91-9640234674',
        email: 'lakshmanv58@gmail.com'
      }
      {
        role: 'Main Coordinator',
        name: 'Dr.K.Satya Sandeep',
        designation: 'Associate Professor- CSE',
        phone: '+91-9985894140',
        email: 'satyasandeepk@gmail.com'
      }
    ],
    facultyMembers: [
      {
        name: 'Ch.Naga Lakshmi',
        designation: 'Assistant Professor - CSM',
        phone: '+91-9000578812',
        email: 'dammavalam.nagalakshmi@gmail.com'
      },
      {
        name: 'Ch.Naga Lakshmi',
        designation: 'Assistant Professor - CSD',
        phone: '+91-9000578812',
        email: 'dammavalam.nagalakshmi@gmail.com'
      },
       {
        name: 'P.Bala Krishna',
        designation: 'Assistant Professor - ECE',
        phone: '+91-7382635228',
        email: 'balakrishna.p12@gmail.com'
      },
      {
        name: 'Y.Vamsi Krishna Teja',
        designation: 'Assistant Professor - IT',
        phone: '+91-8309705324',
        email: 'vamsiyakkala@gmail.com'
      },
      {
        name: 'N.Laxmi Naryana',
        designation: 'Assistant Professor - EEE',
        phone: '+91-9553959485',
        email: 'nelluri.22@gmail.com'
      },
     {
        name: 'T.Yamini',
        designation: 'Assistant Professor - BS&H',
        phone: '+91-7680996240',
        email: 'yamini.t319@gmail.com'
      },
      {
        name: 'M.Sriram',
        designation: 'Assistant Professor - MBA',
        phone: '+91-9515901743',
        email: 'malladi.shiriram@gmail.com'
      }
    ],
  }
};
export const committeeDetails = {
  'Research Coordinator': {
    name: ' Research Coordinator Committee',
    composition: [
      {
        role: 'Chairman',
        name: 'Dr.Radhika Paturi ',
        designation: 'Principal',
        phone: '+91-9866342224',
        email: 'vnits.j@gmail.com'
      },
      {
        role: 'Dean',
        name: 'Dr.V.Lakshman Narayana',
        designation: 'Professor&HOD- CSE',
        phone: '+91-9640234674',
        email: 'lakshmanv58@gmail.com'
      }
      {
        role: 'Main Coordinator',
        name: 'Dr.A.Peda Gopi',
        designation: 'Associate Professor- CSE',
        phone: '+91-9603320281',
        email: 'gopiarepalli2@gmail.com'
      }
    ],
    facultyMembers: [
      {
        name: 'T.Naga Navya',
        designation: 'Assistant Professor - CSM',
        phone: '+91-8179873737',
        email: 'tnavyab@gmail.com'
      },
      {
        name: 'T.Naga Navya',
        designation: 'Assistant Professor - CSD',
        phone: '+91-8179873737',
        email: 'tnavyab@gmail.com'
      },
       {
        name: 'Dr.J.Thrisul Kumar',
        designation: 'Assistant Professor - ECE',
        phone: '+91-9550501443',
        email: 'kumarthrisul9@gmail.com'
      },
      {
        name: 'Dr.V.Pavani',
        designation: 'Assistant Professor - IT',
        phone: '+91-9346405793',
        email: 'manojpavani81@gmail.com'
      },
      {
        name: 'Dr.R.Srikanth',
        designation: 'Associate PROF- EEE',
        phone: '+91-9700445084',
        email: 'ravipatister@gmail.com'
      },
     {
        name: 'Dr.P.Sneha Latha',
        designation: 'Assistant Professor - BS&H',
        phone: '+91-9959440700',
        email: 'vanukuru.sneha@gmail.com'
      },
      {
        name: 'Dr.L.Vijaya',
        designation: 'Assistant Professor - MBA',
        phone: '+91-9985744456',
        email: 'vijaya.sivakiran@gmail.com'
      }
    ],
  }
};
export const committeeDetails = {
  'Enterpreneurship Development cell': {
    name: ' Enterpreneurship Development cell Committee',
    composition: [
      {
        role: 'Chairman',
        name: 'Dr.Radhika Paturi ',
        designation: 'Principal',
        phone: '+91-9866342224',
        email: 'vnits.j@gmail.com'
      },
      {
        role: 'Dean',
        name: 'Dr.V.Lakshman Narayana',
        designation: ' Professor&HOD - CSE',
        phone: '+91-9640234674',
        email: 'lakshmanv58@gmail.com'
      }
      {
        role: 'Main Coordinator',
        name: 'Dr.L.Vijaya',
        designation: 'Assistant Professor - MBA',
        phone: '+91-9985744456',
        email: 'vijaya.sivakiran@gmail.com'
      }
    ],
    facultyMembers: [
       {
        name: 'Dr.A.Peda Gopi',
        designation: 'Associate Professor-CSE',
        phone: '+91-9603320281',
        email: 'gopiarepalli2@gmail.com'
      },
      {
        name: 'Dr.S.Siva Venkata Ramana',
        designation: 'Assistant Professor - CSM',
        phone: '+91-8639882913',
        email: 'sivanirula84@gmail.com'
      },
      {
        name: 'Dr.S.Siva Venkata Ramana',
        designation: 'Assistant Professor - CSD',
        phone: '+91-8639882913',
        email: 'sivanirula84@gmail.com'
      },
       {
        name: 'Dr.P.Sudhakar',
        designation: 'Associate Professor - ECE',
        phone: '+91-7780405914',
        email: ''
      },
      {
        name: 'Y.Vamsi Krishna Teja',
        designation: 'Assistant Professor - IT',
        phone: '+91-8309705324',
        email: 'vamsiyakkala@gmail.com'
      },
      {
        name: 'Ch.Vaisali',
        designation: 'Assistant Professor'- 'EEE',
        phone: '+91-78937097645',
        email: 'vyshunani.pallapu@gmail.com'
      }
    ],
  }
};
export const committeeDetails = {
  'Patents/Startups': {
    name: 'Patents/Startups Committee',
    composition: [
      {
        role: 'Chairman',
        name: 'Dr.Radhika Paturi ',
        designation: 'Principal',
        phone: '+91-9866342224',
        email: 'vnits.j@gmail.com'
      },
      {
        role: 'Dean',
        name: 'Dr.V.Lakshman Narayana',
        designation: ' Professor&HOD - CSE',
        phone: '91-9640234674',
        email: 'lakshmanv58@gmail.com'
      }
      {
        role: 'Main Coordinator',
        name: 'K.Chaitanya',
        designation: 'Associate Professor- CSE',
        phone: '+91-9603956946',
        email: 'chaitanyacns3@gmail.com'
      }
    ],
    facultyMembers: [
      {
        name: 'Dr.J.J.Anand',
        designation: 'Assistant Professor - CSM',
        phone: '+91-9394319440',
        email: 'anand.jupalli@gmail.com'
      },
      {
        name: 'Dr.J.J.Anand',
        designation: 'Assistant Professor - CSD',
        phone: '+91-9394319440',
        email: 'anand.jupalli@gmail.com'
      },
       {
        name: 'Dr.Joseph Sanam',
        designation: 'Professor - ECE',
        phone: '+91-7008265859',
        email: 'joseph.nitr@gmail.com'
      },
      {
        name: 'R.Srinivas',
        designation: 'Assistant Professor - IT',
        phone: '+91-8179715157',
        email: 'srinunaik577@gmail.com'
      },
      {
        name: 'Dr.R.Srikanth',
        designation: 'Associate professor- EEE',
        phone: '+91-9700445084',
        email: 'ravipatister@gmail.com'
      },
     {
        name: 'Dr.P.Sneha Latha',
        designation: 'Professor - BS&H',
        phone: '+91-9959440700',
        email: 'vanukuru.sneha@gmail.com'
      },
      {
        name: 'Dr.L.Vijaya',
        designation: 'Assistant Professor - MBA',
        phone: '+91-9985744456',
        email: 'vijaya.sivakiran@gmail.com'
      }
    ],
  }
};
export const committeeDetails = {
  'Women Empowerment ': {
    name: 'Women Empowerment Committee',
    composition: [
      {
        role: 'Chairman',
        name: 'Dr.Radhika Paturi ',
        designation: 'Principal',
        phone: '+91-9866342224',
        email: 'vnits.j@gmail.com'
      },
      {
        role: 'Dean',
        name: 'Dr.V.Lakshman Narayana',
        designation: 'Professor & HOD - CSE',
        phone:'+91-9640234674',
        email: 'lakshmanv58@gmail.com'
      }
      {
        role: 'Main Coordinator',
        name: 'Dr.V.Sujatha',
        designation: ' Professor- CSE',
        phone: '+91-9182678305',
        email: 'sujiekkurthi@gmail.com'
      }
    ],
    facultyMembers: [
      {
        name: 'Dr.S.Siva Venkata Ramana',
        designation: 'Assistant Professor - CSM',
        phone: '+91-8639882913',
        email: 'sivanirula84@gmail.com'
      },
      {
        name: 'Dr.S.Siva Venkata Ramana',
        designation: 'Assistant Professor - CSD',
        phone: '+91-8639882913',
        email: 'sivanirula84@gmail.com'
      },
       {
        name: 'Dr.B.M.S.Rani',
        designation: 'Professor - ECE',
        phone: '+91-9440746017',
        email: 'ranibms@gmail.com'
      },
      {
        name: 'G.Rohini Phanindra Kumari',
        designation: 'Assistant Professor - IT',
        phone: '+91-8500219323',
        email: 'ginjupallirohini@gmail.com'

      },
      {
        name: 'S.Lakshmi Sirisha',
        designation: 'Assistant professor- EEE',
        phone: '+91-9951687572',
        email: 'slsirisha.s@gmail.com'
      },
     {
        name: 'B.Lavanya',
        designation: 'Assistant Professor - BS&H',
        phone: '+91-9951815410',
        email: 'lavanyabandaru85@gmail.com'
      },
      {
        name: 'Dr.E.Venu Madhavi',
        designation: 'Professor -MBA',
        phone: '+91-9441127829',
        email: 'elurivenumadhavi@gmail.com'
      }
    ],
  }
};
export const committeeDetails = {
  'Hostel Welfare Committee': {
    name: 'Hostel Welfare Committee',
    composition: [
      {
        role: 'Chairman',
        name: 'Dr.Radhika Paturi ',
        designation: 'Principal',
        phone: '+91-9866342224',
        email: 'vnits.j@gmail.com'
      },
      {
        role: 'Dean',
        name: 'Dr.S.Siva Venkata Ramana',
        designation: 'Associate Professor - CSM & CSD',
        phone: '+91-9966667897',
        email: 'sivanirula84@gmail.com'
      }
      {
        role: 'Main Coordinator',
        name: 'Dr.M.Vasumathi Devi',
        designation: 'Associate Professor- CSE',
        phone: '+91-9490729690',
        email: 'mvasudeviravinuthala@gmail.com'
      }
    ],
    facultyMembers: [
      {
        name: 'T.Naga Navya',
        designation: 'Assistant Professor - CSM',
        phone: '+91-8179873737',
        email: 'tnavyab@gmail.com'
      },
      {
        name: 'T.Naga Navya',
        designation: 'Assistant Professor - CSD',
        phone: '+91-8179873737',
        email: 'tnavyab@gmail.com'
      },
       {
        name: 'M.Divya Sree',
        designation: 'Assistant Professor - ECE',
        phone: '+91-9502736002',
        email: 'divyasri.mikkili@gmail.com'
      },
      {
        name: 'B.Aruna Kumari',
        designation: 'Assistant Professor - IT',
        phone: '+91-8184933150',
        email: 'kondaveetiaruna@gmail.com'

      },
      {
        name: 'S.Lakshmi Sirisha',
        designation: 'Assistant professor- EEE',
        phone: '+91-9951687572',
        email: 'slsirisha.s@gmail.com'
      },
     {
        name: 'S.Ratna Kumari',
        designation: 'Assistant Professor - BS&H',
        phone: '+91-9440791062',
        email: 'radhasomi0801@gmail.com'
      },
      {
        name: 'P.Rupavathi',
        designation: 'Assistant Professor - MBA',
        phone: '+91-8555956850',
        email: 'rupa.puttala@gmail.com'
      }
    ],
  }
};
export const committeeDetails = {
  'Discipline Committee': {
    name: 'Discipline Committee',
    composition: [
      {
        role: 'Chairman',
        name: 'Dr.Radhika Paturi ',
        designation: 'Principal',
        phone: '+91-9866342224',
        email: 'vnits.j@gmail.com'
      },
      {
        role: 'Dean',
        name: 'Dr.S.Siva Venkata Ramana',
        designation: 'Associate Professor-CSM&CSD',
        phone: '+91-8639882913',
        email: 'sivanirula84@gmail.com'
      }
      {
        role: 'Main Coordinator',
        name: 'A.Srinivasa Rao',
        designation: 'Assistant Professor- BS&H',
        phone: '+91-9441026791',
        email: 'aresrinivasrao90@gmail.com'
      }
    ],
    facultyMembers: [
      {
        name: 'A.Yugandhar Reddy',
        designation: 'Associate Professor - CSE',
        phone: '+91-9392517845',
        email: 'yugandhar.akkala@gmail.com'
      },
      {
        name: 'Dr.J.J.Anand',
        designation: 'Assistant Professor - CSM',
        phone: '+91-9394319440',
        email: 'anand.jupalli@gmail.com'
      },
      {
        name: 'Dr.J.J.Anand',
        designation: 'Assistant Professor - CSD',
        phone: '+91-9394319440',
        email: 'anand.jupalli@gmail.com'
      },
       {
        name: 'E.Bhanu Prasad',
        designation: 'Assistant Professor- ECE',
        phone: '+91-9912586653',
        email: 'bhanueppea@gmail.com'
      },
      {
        name: 'K.Srikanth',
        designation: 'Assistant Professor - IT',
        phone: '+91-9347489785',
        email: 'sreekilaru@gmail.com'
      },
      {
        name: 'N.Laxmi Naryana',
        designation: 'Assistant professor- EEE',
        phone: '+91-9553959485',
        email: 'nelluri.22@gmail.com'
      },
      {
        name: 'Dr.E.Venu Madhavi',
        designation: 'Professor- MBA',
        phone: '+91-9441127829',
        email: 'elurivenumadhavi@gmail.com'
      }
    ],
  }
};
export const committeeDetails = {
  'Staff Selection Committee': {
    name: 'Staff Selection Committee',
    composition: [
      {
        role: 'Chairman',
        name: 'Dr.Radhika Paturi ',
        designation: 'Principal',
        phone: '+91-9866342224',
        email: 'vnits.j@mail.com'
      },
      {
        role: 'Dean',
        name: 'Dr.S.Siva Venkata Ramana',
        designation: 'Asociate Professor-CSM&CSD',
        phone: '+91-8639882913',
        email: 'sivanirula84@gmail.com'
      }
      {
        role: 'Main Coordinator',
        name: 'Dr.S.Siva Venkata Ramana',
        designation: 'Assistant Professor - CSM',
        phone: '+91-8639882913',
        email: 'sivanirula84@gmail.com'
      }
       {
        role: 'Main Coordinator',
        name: 'Dr.S.Siva Venkata Ramana',
        designation: 'Assistant Professor - CSD',
        phone: '+91-8639882913',
        email: 'sivanirula84@gmail.com'
      }
    ],
    facultyMembers: [
      {
        name: 'Dr.K.Satya Sandeep',
        designation: ' Associate Professor- CSE',
        phone: '+91-9985894140',
        email: 'satyasandeepk@gmail.com'
      },
       {
        name: 'Dr.S.Yalamandaiah',
        designation: 'Associate Professor - ECE',
        phone: '+91-9959587120',
        email: 'yalamandaiah@gmail.com'
      },
      {
        name: 'Dr.V.Pavani',
        designation: 'Assistant Professor - IT',
        phone: '+91-9346405793',
        email: 'manojpavani81@gmail.com'

      },
      {
        name: 'S.Lakshmi Sirisha',
        designation: 'Assistant professor- EEE',
        phone: '+91-9951687572',
        email: 'slsirisha.s@gmail.com'
      },
      {
        name: 'B.Naga Lakshmi',
        designation: 'Assistant Professor- BS&H',
        phone: '+91-9440374283',
        email: 'evurimokshita@gmail.com'
      },
      {
        name: 'Dr.E.Venu Madhavi',
        designation: 'Professor- MBA',
        phone: '+91-9441127829',
        email: 'elurivenumadhavi@gmail.com'
      }
    ],
  }
};
export const committeeDetails = {
  'Governing Body': {
    name: 'Governing Body committee',
    composition: [
      {
        role: 'Chairman',
        name: 'Dr.Radhika Paturi ',
        designation: 'Principal',
        phone: '+91-9866342224',
        email: 'vnits.j@gmail.com'
      },
      {
        role: 'Dean',
        name: 'Dr.S.Siva Venkata Ramana',
        designation: 'Associate Professor - CSD',
        phone: '+91-8639882913',
        email: 'sivanirula84@gmail.com'
      }
      {
        role: 'Main Coordinator',
        name: 'Dr.S.Siva Venkata Ramana',
        designation: 'Assistant Professor - CSM',
        phone: '+91-8639882913',
        email: 'sivanirula84@gmail.com'
      }
       {
        role: 'Main Coordinator',
        name: 'Dr.S.Siva Venkata Ramana',
        designation: 'Assistant Professor - CSD',
        phone: '+91-8639882913',
        email: 'sivanirula84@gmail.com'
      }
    ],
    facultyMembers: [
      {
        name: 'K.Chaitanya',
        designation: ' Associate Professor- CSE',
        phone: '+91-9603956946',
        email: 'chaithanyacns3@gmail.com'
      },
       {
        name: 'D.Jhansi Lakshmi',
        designation: 'Assistant Professor - ECE',
        phone: '+91-9494516439',
        email: 'jhansi_d18@gmail.com'
      },
      {
        name: 'G.Lakshmi Durga',
        designation: 'Assistant Professor - IT',
        phone: '+91-6300298905',
        email: 'gummalakshmi@gmail.com'
      },
      {
        name: 'S.Lakshmi Sirisha',
        designation: 'Assistant professor- EEE',
        phone: '+91-9951687572',
        email: 'slsirisha.s@gmail.com'
      },
      {
        name: 'K.Maha Lakshmi',
        designation: 'Associate Professor- BS&H',
        phone: '+91-9492662284',
        email: 'mhlakshmi@gmail.com'
      },
      {
        name: 'Dr.E.Venu Madhavi',
        designation: 'Professor- MBA',
        phone: '+91-9441127829',
        email: 'elurivenumadhavi@gmail.com'
      }
    ],
  }
};
export const committeeDetails = {
  'Transportation Committee': {
    name: 'Transportation Committee',
    composition: [
      {
        role: 'Chairman',
        name: 'Dr.Radhika Paturi ',
        designation: 'Principal',
        phone: '+91-9866342224',
        email: 'vnits.j@gmail.com'
      },
      {
        role: 'Dean',
        name: 'Dr.S.Siva Venkata Ramana',
        designation: 'Associate Professor-CSM&CSD',
        phone: '+91-8639882913',
        email: 'sivanirula84@gmail.com'
      }
      {
        role: 'Main Coordinator',
        name: 'P.Bala Krishna',
        designation: 'Assistant Professor - ECE',
        phone: '+91-7382635228',
        email: 'balakrishna.p12@gmail.com'
      }
    ],
    facultyMembers: [
      {
        name: 'Dr.A.Naresh',
        designation: 'Professor- CSE',
        phone: '+91-9966135989',
        email: 'alapatinaresh13@gmail.com'
      },
       {
        name: 'A.Chenna Kesava',
        designation: 'Assistant Professor - CSM',
        phone: '+91-8897909053',
        email: 'appi.chennai001@gmail.com'
      },
       {
        name: 'A.Chenna Kesava',
        designation: 'Assistant Professor - CSD',
        phone: '+91-8897909053',
        email: 'appi.chennai001@gmail.com'
      },
      {
        name: 'G.Kusuma Harinath',
        designation: 'Assistant Professor - IT',
        phone: '+91-9052526465',
        email: 'nirulagkhari@gmail.com'
      },
      {
        name: 'T.Rajesh',
        designation: 'Assistant professor- EEE',
        phone: '+91-720398722',
        email: 'thangellaraja232@gmail.com'
      },
      {
        name: 'B.Rajashekar',
        designation: '- BS&H',
        phone: '+91-9951055089',
        email: 'prajasekar99@gmail.com'
      },
      {
        name: 'P.Rupavathi',
        designation: 'Assistant Professor- MBA',
        phone: '+91-8555956850',
        email: 'rupa.puttala@gmail.com'
      }
    ],
  }
};
export const committeeDetails = {
  'Training cordinator Committee': {
    name: 'Training cordinator Committee',
    composition: [
      {
        role: 'Chairman',
        name: 'Dr.Radhika Paturi ',
        designation: 'Principal',
        phone: '+91-9866342224',
        email: 'vnits.j@gmail.com'
      },
      {
        role: 'Dean',
        name: 'Mr.K.Chaitanya',
        designation: 'Associate Professor-CSE',
        phone: '+91-9603956946',
        email: 'chaitanyacns3@gmail.com'
      }
      {
        role: 'Main Coordinator',
        name: 'Dr.V.Pavani',
        designation: 'Assistant Professor - IT',
        phone: '+91-9346405793',
        email: 'manojpavani81@gmail.com'
      }
    ],
    facultyMembers: [
      {
        name: 'Dr.V.Sujatha',
        designation: 'Professor - CSE',
        phone: '9182678305',
        email: 'sujiekkurthi@gmail.com'
      },
       {
        name: 'Dr.D.Anveshini',
        designation: 'Assistant Professor - CSM',
        phone: '+91-9381533055',
        email: 'danveshini@gmail.com'
      },
       {
        name: 'Dr.D.Anveshini',
        designation: 'Assistant Professor - CSD',
        phone: '+91-9381533055',
        email: 'danveshini@gmail.com'
      },
      {
        name: 'K.Vinuthna',
        designation: 'Assistant Professor - ECE',
        phone: '+91-9701967231',
        email: 'kondaveetivinuthna@gmail.com'
      },
      {
        name: 'S.Lakshmi Sirisha',
        designation: 'Assistant professor- EEE',
        phone: '+91-9951687572',
        email: 'slsirisha.s@gmail.com'
      },
      {
        name: 'M.Sriram',
        designation: 'Assistant Professor- MBA',
        phone: '+91-9515901743',
        email: 'malladi.shriram@gmail.com'
      }
    ],
  }
};
export const committeeDetails = {
  'Placement coordinator': {
    name: 'Placement coordinator Committee',
    composition: [
      {
        role: 'Chairman',
        name: 'Dr.Radhika Paturi ',
        designation: 'Principal',
        phone: '+91-9866342224',
        email: 'vnits.j@gmail.com'
      },
      {
        role: 'Dean',
        name: 'Mr.K.Chaitanya',
        designation: 'Associate Professor-CSE',
        phone: '+91-9603956946',
        email: 'chaitanyacns3@gmail.com'
      }
      {
        role: 'Main Coordinator',
        name: 'Dr.B.M.S.Rani',
        designation: 'Professor - ECE',
        phone: '+91-9440746017',
        email: 'ranibms@gmail.com'
      }
    ],
    facultyMembers: [
      {
        name: 'K.Chaitanya',
        designation: 'Associate Professor - CSE',
        phone: '+91-9603956946',
        email: 'chaitanyacns3@gmail.com'
      },
       {
        name: 'M.Prasanna Kumar',
        designation: 'Assistant Professor - CSM',
        phone: '+91-8807149706',
        email: 'himpkumar@gmail.com'
      },
       {
        name: 'M.Prasanna Kumar',
        designation: 'Assistant Professor - CSD',
        phone: '+91-8807149706',
        email: 'himpkumar@gmail.com'
      },
      {
        name: 'P.Sandhya Krishna',
        designation: 'Assistant Professor - IT',
        phone: '+91-9502128165',
        email: 'sandhyamcaviit@gmail.com'
      },
      {
        name: 'S.Lakshmi Sirisha',
        designation: 'Assistant professor- EEE',
        phone: '+91-9951687572',
        email: 'slsirisha.s@gmail.com'
      },
      {
        name: 'M.Sriram',
        designation: 'Assistant professor- MBA',
        phone: '9515901743',
        email: 'malladi.shriram@gmail.com'
      }
    ],
  }
};
export const committeeDetails = {
  'Alumni': {
    name: ' Alumni Committee',
    composition: [
      {
        role: 'Chairman',
        name: 'Dr.Radhika Paturi ',
        designation: 'Principal',
        phone: '+91-9866342224',
        email: 'vnits.j@gmail.com'
      },
      {
        role: 'Dean',
        name: 'Mr.K.Chaitanya',
        designation: 'Associate Professor-CSE',
        phone: '+91-9603956946',
        email: 'chaitanyacns3@gmail.com'
      }
      {
        role: 'Main Coordinator',
        name: 'Dr.D.Anveshini',
        designation: 'Assistant Professor - CSM',
        phone: '+91-9381533055',
        email: 'danveshini@gmail.com'
      }
        {
        role: 'Main Coordinator',
        name: 'Dr.D.Anveshini',
        designation: 'Assistant Professor - CSD',
        phone: '+91-9381533055',
        email: 'danveshini@gmail.com'
      }
    ],
    facultyMembers: [
      {
        name: 'T.Navya Deepthi',
        designation: 'Associate Professor- CSE',
        phone: '+91-9703338124',
        email: 'navya.deepthi6@gmail.com'
      },
      {
        name: 'Dr.D.Anveshini',
        designation: 'Assistant Professor - CSM',
        phone: '+91-9381533055',
        email: 'danveshini@gmail.com'
      },
     
      {
        name: 'Dr.D.Anveshini',
        designation: 'Assistant Professor - CSD',
        phone: '+91-9381533055',
        email: 'danveshini@gmail.com'
      },
      {
        name: 'PK.Vinuthna ',
        designation: 'Assistant Professor - ECE',
        phone: '+91-9701967231',
        email: 'kondaveetivinuthna@gmail.com'
      },
   
      {
        name: 'G.Lakshmi Durga',
        designation: 'Assistant Professor - IT',
        phone: '+91-6300298905',
        email: 'gummalakshmi@gmail.com'
      }
      {
        name: 'S.Lakshmi Sirisha',
        designation: 'Assistant professor- EEE',
        phone: '+91-9951687572',
        email: 'slsirisha.s@gmail.com'
      }
      {
        name: 'Dr.L.Vijaya',
        designation: 'Associate proffesor- MBA',
        phone: '+91-9985744456',
        email: 'vijaya.sivakiran@gmail.com'
      }
    ],
  }
};
export const committeeDetails = {
  'AICTE, NIRF, FFC,NBA, NAAC Committee': {
    name: 'AICTE, NIRF, FFC,NBA, NAAC Committee',
    composition: [
      {
        role: 'Chairman',
        name: 'Dr.Radhika Paturi ',
        designation: 'Principal',
        phone: '+91-9866342224',
        email: 'vnits.j@gmail.com'
      },
      {
        role: 'Dean',
        name: 'P. Silpa Chaitanya',
        designation: 'HOD - CSM & CSD',
        phone: '+91-9849916028',
        email: 'silpam86@gmail.com'
      }
      {
        role: 'Main Coordinator',
        name: 'Dr.D.Naga Bhargavi',
        designation: 'Assistant Professor - CSM',
        phone: '+91-9490514627',
        email: 'hasini.d1984@gmail.com'
      }
       {
        role: 'Main Coordinator',
        name: 'Dr.D.Naga Bhargavi',
        designation: 'Assistant Professor - CSD',
        phone: '+91-9490514627',
        email: 'hasini.d1984@gmail.com'
      }
    ],
    facultyMembers: [
      {
        name: 'Y.Swathi',
        designation: 'Associate Professor - CSE',
        phone: '+91-9490786883',
        email: 'swathiyalavarthy123@gmail.com'
      },
       {
        name: 'M.Divya Sree',
        designation: 'Assistant Professor- ECE',
        phone: '+91-9502736002',
        email: 'divyasri.mikkili@gmail.com'
      },
      {
        name: 'G.Rohini Phanindra Kumari',
        designation: 'Assistant Professor - IT',
        phone: '+91-8500219323',
        email: 'ginjupallirohini@gmail.com'
      },
      {
        name: 'S.Lakshmi Sirisha',
        designation: 'Assistant professor- EEE',
        phone: '+91-9951687572',
        email: 'slsiriha.s@gmail.com'
      },
      {
        name: 'Dr.D.Prasanna',
        designation: 'Professor & HOD- BS&H',
        phone: '+91-9908213405',
        email: 'tynp11@gmail.com'
      },
      {
        name: 'Dr.E.Venu Madhavi',
        designation: 'Proffesor- MBA',
        phone: '9441127829',
        email: 'elurivenumadhavi@gmail.com'
      }
    ],
  }
};
export const committeeDetails = {
  'Academic Audit  Committee': {
    name: 'Academic Audit  Committee',
    composition: [
      {
        role: 'Chairman',
        name: 'Dr.Radhika Paturi ',
        designation: 'Principal',
        phone: '+91-9866342224',
        email: 'vnits.j@gmail.com'
      },
      {
        role: 'Dean',
        name: 'P. Silpa Chaitanya',
        designation: 'HOD - CSM & CSD',
        phone: '+91-9849916028',
        email: 'silpam86@gmail.com'
      }
      {
        role: 'Main Coordinator',
        name: 'Mr.P.Bala krishna',
        designation: 'Assistant Professor - ECE',
        phone: '7382635228',
        email: 'balakrishna.p12@gmail.com'
      }
    ],
    facultyMembers: [
      {
        name: 'Dr.K.Satya Sandeep',
        designation: 'Associate Professor - CSE',
        phone: '+91-9985894140',
        email: 'satyasandeepk@gmail.com'
      },
       {
        name: 'M.Anusha',
        designation: 'Assistant Professor - CSM',
        phone: '+91-8374863576',
        email: 'anusharanimedarametla@gmail.com'
      },
       {
        name: 'T.Naga Navya',
        designation: 'Assistant Professor - CSD',
        phone: '+91-8374863576',
        email: 'anusharanimedarametla@gmail.com'
      },
      {
        name: 'B.Aruna Kumari',
        designation: 'Assistant Professor - IT',
        phone: '+91-8184933150',
        email: 'kondaveetiaruna@gmail.com'
      },
      {
        name: 'T.Rajesh',
        designation: 'Assistant professor- EEE',
        phone: '+91-7207398722',
        email: 'thangellaraja232@gmail.com'
      },
      {
        name: 'Dr.G.Sailaja',
        designation: 'Associate Professor- BS&H',
        phone: '+91-9676355370',
        email: 'sailaja.gannamaneni@gmail.com'
      },
      {
        name: 'M.Saraswathi',
        designation: 'Assistant professor- MBA',
        phone: '9398901719',
        email: 'saraswathinagaraju2011@gmail.com'
      }
    ],
  }
};
export const committeeDetails = {
  'Internal Complaint Committee': {
    name: 'Internal Complaint Committee',
    composition: [
      {
        role: 'Chairman',
        name: 'Dr.Radhika Paturi ',
        designation: 'Principal',
        phone: '+91-9866342224',
        email: 'vnits.j@gmail.com'
      },
      {
        role: 'Dean',
        name: 'P. Silpa Chaitanya',
        designation: 'HOD - CSM & CSD',
        phone: '+91-9849916028',
        email: 'silpam86@gmail.com'
      }
      {
        role: 'Main Coordinator',
        name: 'Dr.G.Sailaja',
        designation: '- BS&H',
        phone: '+91-9676355370',
        email: 'sailaja.gannamaneni@gmail.com'
      }
    ],
    facultyMembers: [
      {
        name: 'C.Kalyana Deepthi',
        designation: ' Associate Professor- CSE',
        phone: '',
        email: 'kalyanid9@gmail.com'
      },
      {
        name: 'Dr.D.Naga Bhargavi',
        designation: 'Assistant Professor - CSM',
        phone: '+91-9490514627',
        email: 'hasini.d1984@gmail.com'
      },
      {
        name: 'Dr.D.Naga Bhargavi',
        designation: 'Assistant Professor - CSD',
        phone: '+91-9490514627',
        email: 'hasini.d1984@gmail.com'
      },
       {
        name: 'K.Hemalatha',
        designation: 'Assistant Professor - ECE',
        phone: '+91-9095035451',
        email: 'bujjihema89@gmail.com'
      },
      {
        name: 'K.Jagadeesh',
        designation: 'Assistant Professor - IT',
        phone: '+91-9885595587',
        email: 'jagadeesh.kalava@gmail.com'
      },
      {
        name: 'S.Lakshmi Sirisha',
        designation: 'Assistant professor- EEE',
        phone: '+91-9951687572',
        email: 'slsirisha.s@gmail.com'
      },
      {
        name: 'Dr.M.Naga Lakshmi',
        designation: 'Associate proffesor- MBA',
        phone: '9700229394',
        email: 'lakshmirafi1997@gami.com'
      }
    ],
  }
};
export const committeeDetails = {
  'Grevience Redressal': {
    name: 'Grevience Redressal Committee',
    composition: [
      {
        role: 'Chairman',
        name: 'Dr.Radhika Paturi ',
        designation: 'Principal',
        phone: '+91-9866342224',
        email: 'vnits.j@gmail.com'
      },
      {
        role: 'Dean',
        name: 'P.Silpa chaitanya',
        designation: 'HOD - CSM & CSD',
        phone: '+91-9849916028',
        email: 'silpam86@gmail.com'
      }
      {
        role: 'Main Coordinator',
        name: 'P.Sandhya Krishna',
        designation: 'Assistant Professor - IT',
        phone: '+91-9502128165',
        email: 'sandhyamcaviit@gmail.com'
      }
    ],
    facultyMembers: [
      {
        name: 'P.Ganga Bhavani',
        designation: 'Associate Professor- CSE',
        phone: '+91-8328663970',
        email: 'bhavanipaturi@gmail.com'
      },
      {
        name: 'Dr.D.Naga Bhargavi',
        designation: 'Assistant Professor - CSM',
        phone: '+91-9490514627',
        email: 'hasini.d1984@gmail.com'
      },
      {
        name: 'Dr.D.Naga Bhargavi',
        designation: 'Assistant Professor - CSD',
        phone: '+91-9490514627',
        email: 'hasini.d1984@gmail.com'
      },
     
      {
        name: 'K.Hemalatha',
        designation: 'Assistant Professor - ECE',
        phone: '+91-9095035451',
        email: 'bujjihema89@gmail.com'
      },
      {
        name: 'S.Lakshmi Sirisha ',
        designation: 'Assistant professor- EEE',
        phone: '+91-9951687572',
        email: 'slsirisha.s@gmail.com'
      },
   
      {
        name: 'K.Neelima',
        designation: 'Assistant Professor- BS&H',
        phone: '+91-9949321466',
        email: 'neelima.pantangi@gmail.com'
      }
       {
        name: 'Dr.M.Naga Lakshmi',
        designation: 'Associate proffesor- MBA',
        phone: '+91-9700229394',
        email: 'lakshmirafi1997@gami.com'
      }
    ],
  }
};
export const committeeDetails = {
  'ISO': {
    name: ' ISO Committee',
    composition: [
      {
        role: 'Chairman',
        name: 'Dr.Radhika Paturi ',
        designation: 'Principal',
        phone: '+91-9866342224',
        email: 'vnits.j@gmail.com'
      },
      {
        role: 'Dean',
        name: 'P.Silpa chaitanya',
        designation: 'HOD - CSM & CSD',
        phone: '+91-9849916028',
        email: 'silpam86@gmail.com'
      }
      {
        role: 'Main Coordinator',
        name: 'M.Prasanna Kumar',
        designation: 'Assistant Professor - CSM',
        phone: '+91-+91-8807149706',
        email: 'himpkumar@gmail.com'
      }
        {
        role: 'Main Coordinator',
        name: 'M.Prasanna Kumar',
        designation: 'Assistant Professor - CSD',
        phone: '+91-8807149706',
        email: 'himpkumar@gmail.com'
      }
    ],
    facultyMembers: [
      {
        name: 'M.Vasumathi Devi',
        designation: 'Associate Professor- CSE',
        phone: '+91-9490729690',
        email: 'mvasudeviravinuthala@gmail.com'
      },
      {
        name: 'K.Murali Krishna',
        designation: 'Assistant Professor - ECE',
        phone: '+91-80174113694',
        email: 'murali.kondaveeti435@gmail.com'
      },
     
      {
        name: 'SK.Sharmila',
        designation: 'Assistant Professor - IT',
        phone: '+91-8309419583',
        email: 'mail2shaiksharmila@gmail.com'
      },
      {
        name: 'T.Rajesh ',
        designation: 'Assistant professor- EEE',
        phone: '+91-7207398722',
        email: 'thangellaraja232@gmail.com'
      },
   
      {
        name: 'K.Maha Lakshmi',
        designation: '- BS&H',
        phone: '+91-9492662284',
        email: 'mhlakshmi@gmail.com'
      }
       {
        name: 'M.Saraswati',
        designation: 'Assistant professor- MBA',
        phone: '+91-9398901719',
        email: 'saraswathinagaraju2011@gmail.com'
      }
    ],
  }
};
export const committeeDetails = {
  'Scholarship Committee': {
    name: 'Scholarship Committee',
    composition: [
      {
        role: 'Chairman',
        name: 'Dr.Radhika Paturi ',
        designation: 'Principal',
        phone: '+91-9866342224',
        email: 'vnits.j@gmail.com'
      },
      {
        role: 'Dean',
        name: 'Dr.T.V. N.Prasanna',
        designation: 'Professor - BH&S',
        phone: '+91-',
        email: ''
      }
      {
        role: 'Main Coordinator',
        name: 'Y. Abshalomu',
        designation: 'Assistant Professor - BS&H',
        phone: '+91-9959760977',
        email: 'abshalomuy@gmail.com'
      }
    ],
    facultyMembers: [
      {
        name: 'M.Chandana',
        designation: 'Associate Professor-CSE',
        phone: '+91-8639158434',
        email: 'chandana.muppalla@gmail.com'
      },
      {
        name: 'B.Adilakshmi',
        designation: 'Assistant Professor - CSM',
        phone: '+91-9603184745',
        email: 'adilakshmi581@gmail.com'
      },
      
      {
        name: 'D.Jhansi Lakshmi',
        designation: 'Assistant Professor - ECE',
        phone: '+91-9494516439',
        email: 'jhansi_d18@yahoo.com'
      },
      {
        name: 'G.Lakshmi Durga ',
        designation: 'Assistant Professor - IT',
        phone: '+91-6300298905',
        email: 'gummalakshmi@gmail.com'
      },
    
      {
        name: 'S.Lakshmi Sirisha ',
        designation: 'Assistant professor- EEE',
        phone: '+91-9951687572',
        email: 'slsirisha.s@gmail.com'
      }
      
    ],
  }
};
export const committeeDetails = {
  'Student Welfare Committee': {
    name: 'Student Welfare Committee',
    composition: [
      {
        role: 'Chairman',
        name: 'Dr.Radhika Paturi ',
        designation: 'Principal',
        phone: '+91-9866342224',
        email: 'vnits.j@gmail.com'
      },
      {
        role: 'Dean',
        name: 'Dr.T.V. N.Prasanna',
        designation: 'Professor & Hod - BS&H',
        phone: '+91-9908213405',
        email: 'tvnp11@gmail.com'
      }
      {
        role: 'Main Coordinator',
        name: 'B.Aruna Kumari',
        designation: 'Assistant Professor - IT',
        phone: '+91-8184933150',
        email: 'kondaveetiaruna@gmail.com'
      }
    ],
    facultyMembers: [
      {
        name: 'M.Satya Vijya',
        designation: 'Associate Professor- CSE',
        phone: '+91-9490283200',
        email: 'satyavijaya.m@gmail.com'
      },
      {
        name: 'B.Adilakshmi',
        designation: 'Assistant Professor - CSD',
        phone: '+91-9603184745',
        email: 'adilakshmi581@gmail.com'
      },
      
      {
        name: 'M.Samrajyam',
        designation: 'Assistant Professor - ECE',
        phone: '+91-9908568210',
        email: 'samrajyamsuresh@gmail.com'
      },
      {
        name: 'Ch.Vaisali ',
        designation: 'Assistant professor- EEE',
        phone: '+91-7893797645',
        email: 'vyshunani.pallapu@gmail.com'
      },
    
      {
        name: 'T.Yamini ',
        designation: 'Assitant Professor- BS&H',
        phone: '+91-7680996240',
        email: 'yamini.t319@gmail.com'
      }
      {
        name: 'V.China Annanda Rao ',
        designation: 'Assitant Professor- BS&H',
        phone: '+91-905828682',
        email: 'anandphysics1984@gmail.com'
      }
       {
        name: 'Dr.L.Vijaya ',
        designation: 'Associate proffesor- MBA',
        phone: '+91-9985744456',
        email: 'vijaya.sivakiran@gmail.com'
      }
    ],
  }
};
export const committeeDetails = {
  'SC/ST Committee': {
    name: 'SC/ST Committee',
    composition: [
      {
        role: 'Chairman',
        name: 'Dr.Radhika Paturi ',
        designation: 'Principal',
        phone: '+91-9866342224',
        email: 'vnits.j@gmail.com'
      },
      {
        role: 'Dean',
        name: 'Dr.T.V. N.Prasanna',
        designation: 'Professor - H&S',
        phone: '+91-9908213405',
        email: 'tynp11@gmail.com'
      }
      {
        role: 'Main Coordinator',
        name: 'M.Samrajyam',
        designation: 'Assistant Professor - ECE',
        phone: '+91-9908568210',
        email: 'samrajyamsuresh@gmail.com'
      }
    ],
    facultyMembers: [
      {
        name: 'M.Francis',
        designation: 'Associate Professor- CSE',
        phone: '+91-7386203636',
        email: 'mulaganifrancis@gmail.com'
      },
      {
        name: 'G.Manidheer BabU',
        designation: 'Assistant Professor - CSM',
        phone: '+91-8985865601',
        email: 'manidheer.gorikapudi@gmail.com'
      },
      
      {
        name: 'R.Srinivas',
        designation: 'Assistant Professor - IT',
        phone: '+91-8179715157',
        email: 'srinunaik577@gmail.com'
      },
      {
        name: 'S.Lakshmi Sirisha ',
        designation: 'Assistant professor- EEE',
        phone: '+91-995168572',
        email: 'slsirisha.s@gmail.com'
      },
    
      {
        name: 'V.China Annanda Rao ',
        designation: 'Assistant Professor- BS&H',
        phone: '+91-905828682',
        email: 'anandphysics1984@gmail.com'
      },
      
       {
        name: 'M.Saraswathi ',
        designation: 'Assistant professor- MBA',
        phone: '+91-9398901719',
        email: 'saraswathinagaraju2011@gmail.com'
      }
    ]
  }
};
export const committeeDetails = {
  'Anti- Ragging Committee': {
    name: 'Anti- Ragging Committee',
    composition: [
      {
        role: 'Chairman',
        name: 'Dr.Radhika Paturi ',
        designation: 'Principal',
        phone: '+91-9866342224',
        email: 'vnits.j@gmail.com'
      },
      {
        role: 'Dean',
        name: 'Dr.T.V. N.Prasanna',
        designation: 'Professor & HOD- H&S',
        phone: '+91-9908213405',
        email: 'tvnp11@gmail.com'
      },
      {
        role: 'Main Coordinator',
        name: ' S.Ratna Kumari',
        designation: '-BS&H',
        phone: '+919440791062',
        email: 'radhasomi0801@gmail.com'
      }
    ],
    facultyMembers: [
      {
        name: 'V.Sujatha',
        designation: 'Professor- CSE',
        phone: '+91-9182678305',
        email: 'sujiekkurthi@gmail.com'
      },
      {
        name: 'Dr.D.Naga Bhargavi',
        designation: 'Assistant Professor - CSM',
        phone: '+91-9490514627',
        email: 'hasini.d1984@gmail.com'
      },
      
      {
        name: 'Dr.P.Sudhakar',
        designation: 'Associate Professor - ECE',
        phone: '+91-7780405914',
        email: 'sudhakar426@gmail.com'
      },
      {
        name: 'G.Rohini Phanindra Kumari',
        designation: 'Assistant Professor - IT',
        phone: '+91-8500219323',
        email: 'ginjupallirohini@gmail.com'
      },
    
      {
        name: 'Ch.Vaisali ',
        designation: 'Assistant professor-EEE',
        phone: '+91-7893797645',
        email: 'vyshunani.pallapu@gmail.com'
      },
      
       {
        name: 'M.Saraswathi ',
        designation: 'Assistant professor- MBA',
        phone: '+91-9398901719',
        email: 'saraswathinagaraju2011@gmail.com
'
      }
    ]
  }
};
export const committeeDetails = {
  'NSS/NCC Committee': {
    name: 'NSS/NCC Committee ',
    composition: [
      {
        role: 'Chairman',
        name: 'Dr.Radhika Paturi ',
        designation: 'Principal',
        phone: '+91-9866342224',
        email: 'vnits.j@gmail.com'
      },
      {
        role: 'Dean',
        name: 'Dr.E.Venu Madhavi',
        designation: 'Professor - H&S',
        phone: '+91-9441127829',
        email: 'elurivenumadhavi@gmail.com'
      },
      {
        role: 'Main Coordinator',
        name: ' Dr.H.Bhavani Naga Prasanna',
        designation: '-BS&H',
        phone: '+91-9704467473',
        email: 'bhavaninps@gmail.com'
      }
    ],
    facultyMembers: [
      {
        name: 'P.Ganga Bhavani',
        designation: 'Associate Professor- CSE',
        phone: '+91-8328663970',
        email: 'bhavanipaturi@gmail.com'
      },
      {
        name: 'V.Manogna',
        designation: 'Assistant Professor - CSM',
        phone: '+91-7671825635',
        email: 'manusri1485@gmail.com'
      },
      
      {
        name: 'R.Naga Prameela',
        designation: 'Assistant Professor - ECE',
        phone: '+91-9398497747',
        email: 'prameelaravuri860@gmail.com'
      },
      {
        name: 'G.Lakshmi Durga',
        designation: 'Assistant Professor - IT',
        phone: '+91-6300298905',
        email: 'gummalakshmi@gmail.com'
      },
    
      {
        name: 'Ch.Vaisali ',
        designation: 'Assistant professor-EEE',
        phone: '+91-7893797645',
        email: 'vyshunani.pallapu@gmail.com'
      },
      
       {
        name: 'P.Rupavathi ',
        designation: 'Assistant professor- MBA',
        phone: '+91-8555956850',
        email: 'rupa.puttala@gmail.com'
      }
    ]
  }
};
export const committeeDetails = {
  'Games / Sports Commitee': {
    name: 'Games / Sports Commitee ',
    composition: [
      {
        role: 'Chairman',
        name: 'Dr.Radhika Paturi ',
        designation: 'Principal',
        phone: '+91-9866342224',
        email: 'vnits.j@gmail.com'
      },
      {
        role: 'Dean',
        name: 'Dr.E.Venu Madhavi',
        designation: 'Professor - H&S',
        phone: '+91-9441127829',
        email: 'elurivenumadhavi@gmail.com'
      },
      {
        role: 'Main Coordinator',
        name: ' Dr.A.Naresh',
        designation: 'Professor-CSE',
        phone: '+91-9966135989',
        email: 'alapatinaresh13@gmail.com'
      }
    ],
    facultyMembers: [
      {
        name: 'A.Chenna Kesava',
        designation: 'Assistant Professor - CSM',
        phone: '+91-8897909053',
        email: 'appi.chennai001@gmail.com'
      },
      {
        name: 'K.Jagadeesh',
        designation: 'Assistant Professor - IT',
        phone: '+91-9885595587',
        email: 'jagadeesh.kalava@gmail.com'
      },
      {
        name: 'T.Rajesh',
        designation: 'Assistant professor-EEE',
        phone: '+91-7207398722',
        email: 'thangellaraja232@gmail.com'
      },
    
      {
        name: 'B.Yedukondal Rao ',
        designation: 'BS&H',
        phone: '+91-6304790224',
        email: 'yedukondalarao@gmail.com'
      },
      
       {
        name: 'P.Rupavathi ',
        designation: 'Assistant professor- MBA',
        phone: '+91-8555956850',
        email: 'rupa.puttala@gmail.com'
      }
    ]
  }
};
export const committeeDetails = {
  'SAC Committee': {
    name: 'SAC Committee',
    composition: [
      {
        role: 'Chairman',
        name: 'Dr.Radhika Paturi ',
        designation: 'Principal',
        phone: '+91-9866342224',
        email: 'vnits.j@gmail.com'
      },
      {
        role: 'Dean',
        name: 'Dr.E.Venu Madhavi',
        designation: 'Professor - H&S',
        phone: '+91-9441127829',
        email: 'elurivenumadhavi@gmail.com'
      },
      {
        role: 'Main Coordinator',
        name: ' K.Jagadeesh',
        designation: 'Assistant Professor - IT',
        phone: '+91-9885595587',
        email: 'jagadeesh.kalava@gmail.com'
      }
    ],
    facultyMembers: [
      {
        name: 'P.Ganga Bhavani',
        designation: 'Associate Professor- CSE',
        phone: '+91-8328663970',
        email: 'bhavanipaturi@gmail.com'
      },
      {
        name: 'B.Adilakshmi',
        designation: 'Assistant Professor - CSM',
        phone: '+91-9603184745',
        email: 'adilakshmi581@gmal.com'
      },
      
      {
        name: 'R.Naga Prameela',
        designation: 'Associate Professor - ECE',
        phone: '+91-9398497747',
        email: 'prameelaravuri860@gmail.com'
      },
      {
        name: 'S.Lakshmi Sirisha',
        designation: 'Assistant professor-EEE',
        phone: '+91-9951687572',
        email: 'slsirisha.s@gmail.com'
      },
    
      {
        name: 'B.Lavanya ',
        designation: 'Assistant Professor-BS&H',
        phone: '+91-9959815410',
        email: 'lavanybandaru85@gmail.com'
      },
      
       {
        name: 'Dr.L.Vijaya ',
        designation: 'Associate proffesor- MBA',
        phone: '+91-9985744456',
        email: 'vijaya.sivakiran@gmail.com'
      }
    ],
  }
};
export const committeeDetails = {
  'Infrastructure Development': {
    name: 'Infrastructure Development Committee',
    composition: [
      {
        role: 'Chairman',
        name: 'Dr.Radhika Paturi ',
        designation: 'Principal',
        phone: '+91-9866342224',
        email: 'vnits.j@gmail.com'
      },
      {
        role: 'Dean',
        name: 'S.Lakshmi Sirisha',
        designation: 'Assistant Professor-EEE',
        phone: '+91-9951687572',
        email: 'slsirisha.s@gmail.com'
      },
      {
        role: 'Main Coordinator',
        name: 'Dr.J.Thrisul Kumar',
        designation: 'associate professor- ECE',
        phone: '+91-9550501443',
        email: 'kumarthrisul9@gmail.com'
      }
    ],
    facultyMembers: [
      {
        name: 'A.Yugandhar Reddy',
        designation: 'Associate Professor- CSE',
        phone: '+91-9392517845',
        email: 'yugandhar.akkala@gmail.com'
      },
      {
        name: 'A.Chenna Kesava',
        designation: 'Assistant Professor - CSM',
        phone: '+91-8897909053',
        email: 'appi.chennai001@gmail.com'
      },
     
      {
        name: 'A.Chenna Kesava',
        designation: 'Assistant Professor - CSD',
        phone: '+91-8897909053',
        email: 'appi.chennai001@gmail.com'
      },
      {
        name: 'G.Kusuma Harinath ',
        designation: 'Assistant Professor - IT',
        phone: '+91-9052526465',
        email: 'nirulagkhari@gmail.com'
      },
   
      {
        name: 'S.Lakshmi Sirisha ',
        designation: 'Assistant professor- EEE',
        phone: '+91-9951687572',
        email: 'slsirisha.s@gmail.com'
      },
      {
        name: 'K.Venkateswarlu ',
        designation: 'Assistant Professor - BS&H',
        phone: '+91-9676706793',
        email: 'ronivenkat14@gmail.com'
      },
       {
        name: 'Dr.L.Vijaya',
        designation: 'Associate Professor- MBA',
        phone: '+91-9985744456',
        email: 'vijaya.sivakiran@gmail.com'
      }
    ],
    studentMembers: [
      {
        name: 'Mr G.Venkata Santosh',
        designation: 'Student - II Year-CSE',
        phone: '+91-8790953761',
        email: 'santosh.05@gmail.com'
      },
      {
        name: 'B.Hamsika Sri Lakshmi Durga Anusha',
        designation: 'Student - I-Year-CSE',
        phone: '+91-8309953431',
        email: '24981a057t@raghuenggcollege.in'
      }
    ]
  }
};
export const committeeDetails = {
  'Purchase Committe': {
    name: 'Purchase Committee',
    composition: [
      {
        role: 'Chairman',
        name: 'Dr.Radhika Paturi ',
        designation: 'Principal',
        phone: '+91-9866342224',
        email: 'vnits.j@gmail.com'
      },
      {
        role: 'Dean',
        name: 'S.Lakshmi Sirisha',
        designation: 'Assistant Professor-EEE',
        phone: '+91-9951687572',
        email: 'slsrisha.s@gmail.com'
      },
      {
        role: 'Main Coordinator',
        name: 'Dr.D.Prasanna',
        designation: 'Professor- BS&H',
        phone: '+91-9908213405',
        email: 'tynp11@gmail.com'
      }
    ],
    facultyMembers: [
      {
        name: 'Syed Shareena Bhanu',
        designation: 'Associate Professor- CSE',
        phone: '+91-7032721962',
        email: 'syesshareenabanu@gmail.com'
      },
      {
        name: 'Ch.Naga Lakshmi',
        designation: 'Assistant Professor - CSM',
        phone: '+91-9000578812',
        email: 'dammavalam.nagalakshmi@gmail.com'
      },
     
      {
        name: 'Ch.Naga Lakshmi',
        designation: 'Assistant Professor - CSD',
        phone: '+91-9000578812',
        email: 'dammavalam.nagalakshmi@gmail.com'
      },
      {
        name: 'E.Bhanu Prasad ',
        designation: 'assistant professor- ECE',
        phone: '+91-9912586653',
        email: 'bhanueppea@gmail.com'
      },
   
      {
        name: 'G.Rohini Phanindra Kumari',
        designation: 'Assistant Professor - IT',
        phone: '+91-8500219323',
        email: 'ginjupallirohini@gmail.com'
      },
      {
        name: 'S.Lakshmi Sirisha',
        designation: 'Assistant professor- EEE',
        phone: '+91-9951687572',
        email: 'slsirisha.s@gmail.com'
      },
       {
        name: 'Dr.E.Venu Madhavi',
        designation: 'Proffesor- MBA',
        phone: '+91-9441127829',
        email: 'elurivenumadhavi@gmail.com'
      }
    ]
  }
};
export const committeeDetails = {
  'Finance Committe': {
    name: 'Finance Committee',
    composition: [
      {
        role: 'Chairman',
        name: 'Dr.Radhika Paturi ',
        designation: 'Principal',
        phone: '+91-9866342224',
        email: 'vnits.j@gmail.com'
      },
      {
        role: 'Dean',
        name: 'S.Lakshmi Sirisha',
        designation: 'Assistant Professor-EEE',
        phone: '+91-9951687572',
        email: 'slsirisha.s@gmail.com'
      },
      {
        role: 'Main Coordinator',
        name: 'S.Lakshmi Sirisha',
        designation: 'Assistant Professor- EEE',
        phone: '+91-9951687572',
        email: 'slsirisha.s@gmail.com'
      }
    ],
    facultyMembers: [
      {
        name: 'Y.Swathi',
        designation: 'Associate Professor- CSE',
        phone: '+91-9490786883',
        email: 'swaathiyalavarthy123@gmail.com'
      },
      {
        name: 'Ch.Naga Lakshmi',
        designation: 'Assistant Professor - CSM',
        phone: '+91-9000578812',
        email: 'dammavalam.nagalakshmi@gmail.com'
      },
     
      {
        name: 'Ch.Naga Lakshmi',
        designation: 'Assistant Professor - CSD',
        phone: '+91-9000578812',
        email: 'dammavalam.nagalakshmi@gmail.com'
      },
      {
        name: 'M.Venkat Subba Rao ',
        designation: 'assistant profressor- ECE',
        phone: '+91-6304947590',
        email: 'venkat.manchala111@gmail.com'
      },
   
      {
        name: 'K.Srikanth',
        designation: 'Assistant Professor - IT',
        phone: '+91-9347489785',
        email: 'sreekilaru@gmail.com'
      },
      {
        name: 'J.Venkat Lakshmi',
        designation: '- BS&H',
        phone: '+91-',
        email: ''
      },
       {
        name: 'Dr.E.Venu Madhavi',
        designation: 'Proffesor- MBA',
        phone: '+91-9441127829',
        email: 'elurivenumadhavi@gmail.com'
      }
    ],
    studentMembers: [
      {
        name: 'Mr G.Venkata Santosh',
        designation: 'Student - II Year-CSE',
        phone: '+91-8790953761',
        email: 'santosh.05@gmail.com'
      },
      {
        name: 'B.Hamsika Sri Lakshmi Durga Anusha',
        designation: 'Student - I-Year-CSE',
        phone: '+91-8309953431',
        email: '24981a057t@raghuenggcollege.in'
      }
    ]
  }
};
export const committeeDetails = {
  'Electrical Maintanance': {
    name: 'Electrical Maintanance Committee',
    composition: [
      {
        role: 'Chairman',
        name: 'Dr.Radhika Paturi ',
        designation: 'Principal',
        phone: '+91-9866342224',
        email: 'vnits.j@gmail.com'
      },
      {
        role: 'Dean',
        name: 'S.Lakshmi Sirisha',
        designation: 'Assistant Professor-EEE',
        phone: '+91-9951687572',
        email: 'slsirisha.s@gmail.com'
      },
      {
        role: 'Main Coordinator',
        name: 'Dr.R.Srikanth',
        designation: 'Associate professor- EEE',
        phone: '+91-9700445084',
        email: 'ravipatister@gmail.com'
      }
    ],
    facultyMembers: [
      {
        name: 'M.Francis',
        designation: 'Associate Professor- CSE',
        phone: '+91-7386203636',
        email: 'mulaganifrancis@gmail.com'
      },
      {
        name: 'Ch.Naga Lakshmi',
        designation: 'Assistant Professor - CSM',
        phone: '+91-9000578812',
        email: 'dammavalam.nagalakshmi@gmail.com'
      },
     
      {
        name: 'Ch.Naga Lakshmi',
        designation: 'Assistant Professor - CSD',
        phone: '+91-9000578812',
        email: 'dammavalam.nagalakshmi@gmail.com'
      },
      {
        name: 'M.Venkat Subba Rao ',
        designation: 'assistant profressor- ECE',
        phone: '+91-6304947590',
        email: 'venkat.manchala111@gmail.com'
      },
   
      {
        name: 'K.Srinivas',
        designation: 'Assistant Professor - IT',
        phone: '+91-8179715157',
        email: 'srinunaik577@gmail.com'
      },
      {
        name: 'D.Ratna Mani',
        designation: 'Assistant Professor- BS&H',
        phone: '+91-8019069042',
        email: 'd.ratnamani7@gmail.com'
      },
       {
        name: 'P.Rupavathi',
        designation: 'Assistant professor- MBA',
        phone: '+91-8555956850',
        email: 'rupa.puttala@gmail.com'
      }
    ],
    studentMembers: [
      {
        name: 'Mr G.Venkata Santosh',
        designation: 'Student - II Year-CSE',
        phone: '+91-8790953761',
        email: 'santosh.05@gmail.com'
      },
      {
        name: 'B.Hamsika Sri Lakshmi Durga Anusha',
        designation: 'Student - I-Year-CSE',
        phone: '+91-8309953431',
        email: '24981a057t@raghuenggcollege.in'
      }
    ]
  }
};
export const committeeDetails = {
  'Internal & External Examinations': {
    name: ' Internal & External Examinations Committee',
    composition: [
      {
        role: 'Chairman',
        name: 'Dr.Radhika Paturi ',
        designation: 'Principal',
        phone: '+91-9866342224',
        email: 'vnits.j@gmail.com'
      },
      {
        role: 'Dean',
        name: 'MR.N.Laxmi Narayana',
        designation: '',
        phone: '+91-',
        email: ''
      },
      {
        role: 'Main Coordinator',
        name: 'Dr.J.J.Anand',
        designation: 'Assistant Professor - CSM',
        phone: '+91-9394319440',
        email: 'anand.jupalli@gmail.com'
      }
    ],
    facultyMembers: [
      {
        name: 'P.Ganga Bhavani',
        designation: 'Associate Professor- CSE',
        phone: '+91-8328663970',
        email: 'bhavanipaturi@gmail.com'
      },
      {
        name: 'Ch.Naga Lakshmi',
        designation: 'Assistant Professor - CSM',
        phone: '+91-9000578812',
        email: 'dammavalam.nagalakshmi@gmail.com'
      },
     
      {
        name: 'Ch.Naga Lakshmi',
        designation: 'Assistant Professor - CSD',
        phone: '+91-9000578812',
        email: 'dammavalam.nagalakshmi@gmail.com'
      },
      {
        name: 'P.Ganesh Babu ',
        designation: 'assistant professor - ECE',
        phone: '+91-8985185747',
        email: 'ganeshbabu.pantangi@gmail.com'
      },
   
      {
        name: 'Y.Vamsi Krishna Teja',
        designation: 'Assistant Professor - IT',
        phone: '+91-8309705324',
        email: 'vamsiyakkala@gmail.com'
      },
      {
        name: 'T.Rajesh',
        designation: 'Assistant professor- EEE',
        phone: '+91-7207398722',
        email: 'thangellaraja232@gmail.com '
      },
       {
        name: 'B.Naga Lakshmi',
        designation: 'Assistant Professor- BS&H',
        phone: '+91-9398606604',
        email: 'nagalakshmiborra1@gmail.com'
      }
      {
        name: 'Dr.L.Vijaya',
        designation: 'Associate proffesor- MBA',
        phone: '+91-9985744456',
        email: 'vijaya.sivakiran@gmail.com'
      }
    ],
  }
};
export const committeeDetails = {
  'Acadamic planning': {
    name: ' Acadamic planning Committee',
    composition: [
      {
        role: 'Chairman',
        name: 'Dr.Radhika Paturi ',
        designation: 'Principal',
        phone: '+91-9866342224',
        email: 'vnits.j@gmail.com'
      },
      {
        role: 'Dean',
        name: 'Dr.G.Sandhya',
        designation: 'Professor&HOD-ECE',
        phone: '+91-6300956881',
        email: 'sandhyag406@gmail.com'
      },
      {
        role: 'Main Coordinator',
        name: 'P.Ganesh Babu',
        designation: 'ECE',
        phone: '+91-8985185747',
        email: 'ganeshbabu.pantangi@gmail.com'
      }
        ],
    facultyMembers: [
      {
        name: 'Dr.M.Vasumathi Devi',
        designation: 'Associate Professor- CSE',
        phone: '+91-9490729690',
        email: 'mvasudeviravinuthala@gmail.com'
      },
      {
        name: 'M.Anusha Rani',
        designation: 'Associate Professor- CSM',
        phone: '+91-8374863576 ',
        email: 'anusharanimedarametla@gmail.com'
      },
     
      {
        name: 'P.Sandhya Krishna',
        designation: 'Assosiate Professor- IT',
        phone: '+91-9502128165',
        email: 'sandhyamcaviit@gmail.com'
      },
      {
        name: 'K.Maha Lakshmi ',
        designation: 'Assosiate Professor- BS&H',
        phone: '+91-9492662284',
        email: 'mhlakahmi@gmail.com'
      },
   
      {
        name: 'Dr.L.Vijaya',
        designation: 'Professor - MBA',
        phone: '+91-9985744456',
        email: 'vijaya.sivakiran@gmail.com'
      },
      {
        name: 'T.Rajesh',
        designation: 'Assistant professor- EEE',
        phone: '+91-7207398722',
        email: 'thangellaraja232@gmail.coM'
      }
    }
};
